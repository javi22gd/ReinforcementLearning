...
def is_natural(hand):
    """
    Indica si una mano contiene un natural:
    Dada una mano, devuelve true si hay un ace y una figura; false en
    caso contrario.
    """
...
class BlackjackEnv(gym.Env):
    ...
    def _step(self, action):
        ...
        Si el jugador tiene un natural y el dealer no, se termina la
        partida con recompensa = 1.5; si ambos tienen un natural, se
        termina con recompensa = 0 (empate); en otro caso, se continúa.
        ...
        if self.natural_player and not self.natural_dealer:
            done = True
            reward = 1.5
        elif self.natural_player and self.natural_dealer:
            done = True
            reward = 0
        else:
    ...
    def _reset(self):
        """
        Reparte una mano al jugador (hasta que la suma de sus cartas sea mayor o 
        igual a 12) y al dealer. Devuelve la observación.
        """
        self.dealer = draw_hand(self.np_random)
        self.player = draw_hand(self.np_random)
        
        self.natural_player = is_natural(self.player)
        self.natural_dealer = is_natural(self.dealer)
